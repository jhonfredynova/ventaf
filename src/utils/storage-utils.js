...
export const deleteFromStorage = async data => {
	...
	const foldersToDelete = Array.isArray(data.folderPaths)
		? data.folderPaths
		: [];
	let deletedPaths = [];
	...
	if (foldersToDelete.length > 0) {
		...
		const deletePromises = foldersToDelete.map(
			folderPath =>
				new Promise((resolve, reject) => {
					// removing first slash because issues with storage
					newFolderPath = folderPath.startsWith('/')
						? folderPath.substring(1)
						: folderPath;
					bucket
						.deleteFiles({ prefix: newFolderPath })
						.then(() => {
...
export const uploadToStorage = async data => {
	...
	const uploadFile = async (filePath, storageOptions) => {
		...
		const fileBucketDestination = storageOptions.bucketPath
			? `${storageOptions.bucketPath}/${uploadedFileName}`
			: uploadedFileName;
		const uploadResponse = await bucket.upload(filePath, {
			destination: fileBucketDestination
		});
