...
export const getDbQuery = async (db, collection, query) => {
  ...
  response = response.docs.map((doc) => (Object.assign(doc.data(), { id: doc.id })));
...
export const getDbQueryByIds = async (db, collection, documentIds) => {
  let response = [];
  const documentPromises = documentIds.filter((id) => id).map((id) => db.collection(collection).doc(id).get());

  response = await Promise.all(documentPromises);
  response = response.filter((doc) => !doc.isEmpty).map((doc) => (Object.assign(doc.data(), { id: doc.id })));
