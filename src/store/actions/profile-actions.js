...
export const getProfileAds = (profileId, filters) => async (
	dispatch,
	getState
) => {
...
export const getProfileByEmail = email => async dispatch => {
	const response = await axios.post(
		`${process.env.NEXT_PUBLIC_API_URL}/users/get-profile-by-email?email=${email}`
	);
...
export const getProfileById = userId => async (dispatch, getState) => {
	...
	if (!profile) {
		profile = await axios.post(
			`${process.env.NEXT_PUBLIC_API_URL}/users/get-profile-by-id?userId=${userId}`
		);
...
export const getProfileByUsername = username => async (dispatch, getState) => {
	...
	if (!profile) {
		profile = await axios.post(
			`${process.env.NEXT_PUBLIC_API_URL}/users/get-profile-by-username?username=${username}`
		);
		profile = profile.data;
	}
