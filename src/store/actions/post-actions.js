...
export const getMorePosts = query => async dispatch => {
  const posts = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/posts/get-all${setUrlSearch(query)}`);
...
export const getPostById = postId => async (dispatch, getState) => {
  ...
  const allPosts = posts.concat(profilePosts).concat(currentPost).filter(post => post);
  let postData = allPosts.find(post => post.id === postId);

  if (!postData) {
    postData = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/posts/${postId}`);
...
export const getRelatedContent = postId => async dispatch => {
  const posts = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/posts/get-related-content?postId=${postId}`);
  dispatch({ type: TYPES.GET_RELATED_CONTENT, payload: posts.data });
};

export const createPost = postData => async dispatch => {
  const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/posts/create`, postData);
  dispatch({ type: TYPES.CREATE, payload: response.data });
  return response.data;
};

export const updatePost = (postId, postData) => async dispatch => {
  const response = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/posts/update?postId=${postId}`, postData);
  dispatch({ type: TYPES.UPDATE, payload: response.data });
};

export const updatePostViews = postId => async dispatch => {
  const response = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/posts/update-views?postId=${postId}`);
  dispatch({ type: TYPES.UPDATE_VIEWS, payload: response.data });
};

export const deletePost = postId => async dispatch => {
  const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/posts/remove?postId=${postId}`);
