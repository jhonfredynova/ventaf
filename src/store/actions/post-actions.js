...
export const getPosts = filters => async dispatch => {
	const response = await axios.get(
		`${process.env.NEXT_PUBLIC_API_URL}/posts/get-all${setUrlSearch(
			filters
		)}`
	);
...
export const getMorePosts = query => async dispatch => {
	const posts = await axios.get(
		`${process.env.NEXT_PUBLIC_API_URL}/posts/get-all${setUrlSearch(query)}`
	);
...
export const getPostById = postId => async (dispatch, getState) => {
	...
	const profilePosts = Object.keys(profiles).flatMap(
		profileId => profiles[profileId].ads || []
	);
...
export const getRelatedContent = postId => async dispatch => {
	const posts = await axios.get(
		`${process.env.NEXT_PUBLIC_API_URL}/posts/get-related-content?postId=${postId}`
	);
...
export const updatePost = (postId, postData) => async dispatch => {
	const response = await axios.patch(
		`${process.env.NEXT_PUBLIC_API_URL}/posts/update?postId=${postId}`,
		postData
	);
...
export const updatePostViews = postId => async dispatch => {
	const response = await axios.patch(
		`${process.env.NEXT_PUBLIC_API_URL}/posts/update-views?postId=${postId}`
	);
...
export const deletePost = postId => async dispatch => {
	const response = await axios.delete(
		`${process.env.NEXT_PUBLIC_API_URL}/posts/remove?postId=${postId}`
	);
